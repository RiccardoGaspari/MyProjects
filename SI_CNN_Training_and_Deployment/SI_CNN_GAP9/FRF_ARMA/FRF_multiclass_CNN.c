...
AT_DEFAULTFLASH_EXT_ADDR_TYPE FRF_multiclass_CNN_L3_Flash = 0;
...
/* Inputs */
L2_MEM unsigned char Input_1[819];
/* Outputs */
...
int copy_inputs(int num_iterations) {
    {
        ...
        ret_Input_1 = __READ(Input_File_Input_1, Input_1, 819);
        if (ret_Input_1 != 819) {
            return 0;
        }
        Input_File_Input_1_Position = 0;
...
static void cluster(void * arg)
{
    ...
    FRF_multiclass_CNNCNN_ConstructCluster();
GPIO_HIGH();
    FRF_multiclass_CNNCNN(Input_1, Output_1);
GPIO_LOW();
...
{
    printf("\n\n\t *** NNTOOL FRF_multiclass_CNN Example ***\n\n");
    ...
    printf("Constructor\n");
    int ConstructorErr = FRF_multiclass_CNNCNN_Construct();
    if (ConstructorErr)
    {
    ...
    FRF_multiclass_CNNCNN_Destruct();
